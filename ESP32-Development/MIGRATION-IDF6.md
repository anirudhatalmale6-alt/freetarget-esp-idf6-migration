...
```
freeETarget.bin binary size 0x145330 bytes. Smallest app partition is 0x200000 bytes.
0xbacd0 bytes (36%) free.
...
There are 62 markers across 28 files. Most are explanatory - they say what
moved and why. **Four need a decision from you**; they are marked
`*** PLEASE LOOK AT THIS ONE ***` in the source and listed first below.

This package got the same treatment as `master`, so the items there apply
here too and are not repeated in detail. There is **one extra item that is
specific to this branch** and it is the most important thing in this document
- it is item 4 below.
...
Binary size moved by 16 bytes, from `0x145340` to `0x145330`.
...
This package's `settings.json` was already pointing at
`C:\esp\v6.0.2\esp-idf` with an empty `compilerPath`, so the engine setting
was the only thing that needed changing here. (In `master` those two were
still on the 5.x paths; I corrected them there to match this one.)
...
### 4. `main/json.h` - `json_rapid_time` and `json_rapid_wait` were `int`
