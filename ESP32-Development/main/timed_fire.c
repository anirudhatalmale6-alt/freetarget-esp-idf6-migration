...
static const rapid_state_t *rapid_state = NULL; // TODO(IDF6): const - it points into the const course_of_fire[] tables  // What state table to use
...
static bool timed_fire_start_new_cycle(void)
{
  ...
  // TODO(IDF6): %d -> %4.2f for the last two. They are real_t now (see the
  // note on json_rapid_time in json.h), and your master package prints them
  // with %4.2f too. Printing a double through %d is undefined behaviour,
  // not just a wrong-looking number.
