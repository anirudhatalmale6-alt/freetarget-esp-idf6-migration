...
void read_nonvol(void)
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "read_nonvol()");))
  ...
  if ( err != 0 )
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "read_nonvol(): Failed to initialize NVM");))
  ...
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "read_nonvol(): Failed to open NVM");))
  }
...
void factory_nonvol(bool do_calibration) // TRUE if we are doing a factory calibration
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "factory_nonvol(%d)\r\n", do_calibration);))
  ...
  {
    DLT(DLT_CRITICAL, SEND(CONSOLE, sprintf(_xs, "Failed to write factory defaults to NONVOL");))
  }
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "Factory Init complete\r\n");))
  return;
}
...
void init_nonvol(int verify) // Verification code entered by user
{
   ...
   */
  if ( prompt_for_confirm("Are you sure you want to initialize the unit? (Y/N): ") == false )
  {
    SEND(ALL, sprintf(_xs, "\r\nInitialization cancelled\r\n");)
    return;
  }

  factory_nonvol(prompt_for_confirm("\r\nReset Serial Number")); // Reset to factory defaults and prompt for serial number
...
void update_nonvol(unsigned int current_version) // Version present in persistent storage
{
  ...
  DLT(DLT_INFO, SEND(CONSOLE, sprintf(_xs, "update_nonvol(%d)\r\n", current_version);))
...
void nonvol_write_i32(char *name, int *value) // Name of the value to write
{
  ...
  // The identical bug is in your master and Trace packages.
