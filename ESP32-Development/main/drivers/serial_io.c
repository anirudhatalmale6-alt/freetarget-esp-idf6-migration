...
/*******************************************************************************
 *
 * @function: aux_port_pass_through
 *
 * @brief:    Connect the console to the AUX port
 *
 * @return:   None
 *
 *******************************************************************************
 ...
 *
 ******************************************************************************/
void aux_port_pass_through(void)
{
  unsigned char ch;
  ...
  else
  {
    while ( 1 )
    {
      while ( serial_available(CONSOLE) != 0 )
      {
        ...
        serial_putch(ch, CONSOLE | AUX_PORT);
        if ( ch == '!' )
        {
          break; // Exit on !
        }
      ...
      while ( serial_available(AUX_PORT) != 0 )
      {
        ch = serial_getch(AUX_PORT);
        serial_putch(ch, CONSOLE | AUX_PORT);
      }
