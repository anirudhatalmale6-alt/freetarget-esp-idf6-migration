...
void app_main(void)
{
  ...
  xTaskCreate(server_accept, "server_accept", K4, NULL, POLLING, NULL);   // Wait for an incoming connection
  serial_flush(ALL);
  vTaskDelay(TICK_10ms);
  xTaskCreate(server_receive, "server_receive", K4, NULL, POLLING, NULL); // Wait for incoming data
  serial_flush(ALL);
  vTaskDelay(TICK_10ms);
